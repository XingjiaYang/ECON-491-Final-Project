...
From the output, we can see that the Ridge model has lowest RMSE, which means its predicting is best.
...
I will not do PCA since it is unsupervised learning method. PCA's main goal is to preserve most of variance of predictors when we do Dimensional reduction. The unsupervised learning algorithm will not be helpful in predicting `INTRATE`, therefore I will not do it.

# Summary

In this unscaled data, for predicting, the final model is boosting tree.
...
From the output, I can see that the scaled elastic net model is the best, also better than unscaled mlr shrinkage model. But the model is worse than the boosting tree. Therefore, until now, boosting tree is still the best model.
